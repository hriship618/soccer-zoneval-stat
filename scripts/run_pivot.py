...
from scripts.train_baselines import metrics, train_model
...
def main() -> None:
    ...
    parser.add_argument("--statsbomb", type=Path, default=Path("data/raw/statsbomb"))
    parser.add_argument("--dfl", type=Path, default=Path("data/raw/dfl"))
    parser.add_argument("--output", type=Path, default=Path("data/processed/pivot"))
    parser.add_argument("--dashboard", type=Path, default=Path("app/pivot-rankings.generated.ts"))
    ...
    methodology = {
        ...
        "score_definition": {
            ...
            "aggregation": "C_i = sum_e C_i,e; rate_i = 100*C_i/N_i; shrink rate_i toward the event-sample-weighted population mean with reliability N_i/(N_i+100).",
            "rating": "PIVOT_i = clip(50 + 10*z_group(shrunk rate_i), 0, 100) among players with at least 30 minutes and 30 aligned event samples; goalkeeper and outfield distributions are standardized separately because removal effects are position structurally different.",
        },
        "tracking_features": [
        ...
        "normalization": "event-exposure rate plus empirical-Bayes shrinkage, then separate goalkeeper/outfield standardization; minutes and sample count are retained in every ranking row",
    }
    report = {
        ...
        "ranking": {
            ...
            "qualification": "at least 30 tracked minutes and 30 synchronized event samples",
            "top_10": rankings[:10],
        },
        "limitations": [
    ...
    print(json.dumps({
        ...
        "top_10": [{"rank": row["rank"], "name": row["name"], "team": row["team"], "rating": row["pivot_rating"]} for row in rankings[:10]],
