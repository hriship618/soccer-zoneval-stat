import realMatch from './match-data.generated';
...
export type MatchAction = {
  player: string; type: string; value: number; minute: number;
  from: number; to: number; detail: string;
};

const roles: Record<string,string> = { TW:'GK', IVR:'CB', IVL:'CB', IVZ:'CB', RV:'RB', LV:'LB', DMR:'DM', DML:'DM', DRM:'DM', DLM:'DM', ZO:'AM', RA:'RW', LA:'LW', ORM:'RW', OLM:'LW', STZ:'ST', HR:'RM', HL:'LM', SUB:'SUB' };
...
export const actions: MatchAction[] = realMatch.actions.map(a => ({...a, detail:a.detail}));
